...
public class MultiAroundTest {
    @Pointcut("execution(java.lang.String com.example.bigdata.service.DemoService.sysGuid(..)) ")
    ...
    public Object test1(ProceedingJoinPoint pjp)
        ...
        System.out.println(Thread.currentThread().getName()+"test1:"+m + "用时:" + t);
        return val;
    }
    @Around("pointcut()")
    @Order(2)
    public Object test2(ProceedingJoinPoint pjp)
            throws Throwable {
        MethodSignature signature = (MethodSignature) pjp.getSignature();
        Method method = signature.getMethod(); //获取被拦截的方法
        ...
        Signature m = pjp.getSignature();
        //Signature: 签名, 这里是方法签名
        System.out.println(Thread.currentThread().getName()+"test2:"+m + "用时:" + t);
        return val;
    }
